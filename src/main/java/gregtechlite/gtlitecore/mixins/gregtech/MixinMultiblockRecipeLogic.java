...
public abstract class MixinMultiblockRecipeLogic extends AbstractRecipeLogic
{
    ...
    @WrapOperation(method = "getInputTank(Lnet/minecraftforge/items/IItemHandler;)Lgregtech/api/capability/IMultipleTankHandler;",
                   at = @At(value = "INVOKE",
                            target = "Ljava/util/ArrayList;addAll(Ljava/util/Collection;)Z"))
    private <E> boolean addTanksIfNotWrapped(ArrayList<E> tanks, Collection<E> toAdd, Operation<Boolean> original)
    {
        final ObjectList<E> filtered = new ObjectArrayList<>();
        for (E tank : toAdd)
        {
            if (tank instanceof IMultipleTankHandler.ITankEntry
                    && gtlitecore$isTankAlreadyPresent(tanks, (IMultipleTankHandler.ITankEntry) tank))
            {
                continue;
            }
            filtered.add(tank);
        }
        return original.call(tanks, filtered);
    }
