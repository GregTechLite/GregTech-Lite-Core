                                        ...
                                        val generator: AbstractWorldGenerator) : WorldGenerator(isNotified)
{
    ...
    override fun generate(worldIn: World, rand: Random, pos: BlockPos): Boolean
        = scatter(worldIn, BlockPos.MutableBlockPos(pos), rand)

    open fun setBlockAndUpdate(worldIn: World?, blockPos: BlockPos?, blockState: IBlockState?)
    {
    ...
    open fun getAmountInChunk(conditions: MutableList<GenerateCondition>,
                              chunkX: Int, chunkZ: Int, worldIn: World, blockPos: BlockPos): Int
    {
        ...
        val treeStrength = generator.getRandomStrength(chunkX, chunkZ)

        if (ConfigHolder.misc.debug)
        {
            if (relevantCondition.getPerlinCutoff(biome) < treeStrength)
            {
                generator.updateForPlaced(true)
            ...
            {
                generator.updateForPlaced(false)
            }
        }
        else
        {
                        ...
                        chunkX: Int, chunkZ: Int): Boolean
    {
        ...
        var count = getAmountInChunk(generator.conditions, chunkX, chunkZ, worldIn, blockPos)
        if (count > 0)
        {
            var i = 0
            while (i < count)
            {
                // Setup block pos for spawn offset by 8 to prevent cascading.
                blockPos.setPos(chunk.x * 16 + rand.nextInt(16) + 8,
                                255,
                                chunk.z * 16 + rand.nextInt(16) + 8)

                // Air checking and set Y-axis.
                while (worldIn.isAirBlock(blockPos) && blockPos.y != 0)
