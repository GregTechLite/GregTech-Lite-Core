...
object GTLiteMetaOreDictItems
{
    ...
    fun register()
    {
        CORN_KERNEL = ORE_DICT_ITEMS.addOreDictItem(1, "corn_kernel",
            0xFFEA70, DULL, gemChipped)

        CORN_COB = ORE_DICT_ITEMS.addOreDictItem(2, "corn_cob",
            0xF2D434, DULL, gemFlawed)

        BARE_CORN_KERNEL = ORE_DICT_ITEMS.addOreDictItem(3, "bare_corn_kernel",
            0xFECB60, DULL, gemChipped)

        SMALL_GREEN_COFFEE = ORE_DICT_ITEMS.addOreDictItem(4, "small_green_coffee",
            0x3B220D, DULL, round)

        LARGE_GREEN_COFFEE = ORE_DICT_ITEMS.addOreDictItem(5, "large_green_coffee",
            0x3B220D, DULL, gemChipped)

        FERMENTED_SMALL_COFFEE = ORE_DICT_ITEMS.addOreDictItem(6, "fermented_small_coffee",
            0x756452, DULL, round)

        FERMENTED_LARGE_COFFEE = ORE_DICT_ITEMS.addOreDictItem(7, "fermented_large_coffee",
            0x756452, DULL, gemChipped)

        DRIED_SMALL_COFFEE = ORE_DICT_ITEMS.addOreDictItem(8, "dried_small_coffee",
            0x8C6842, DULL, round)

        DRIED_LARGE_COFFEE = ORE_DICT_ITEMS.addOreDictItem(9, "dried_large_coffee",
            0x8C6842, DULL, gemChipped)

        ROASTED_SMALL_COFFEE = ORE_DICT_ITEMS.addOreDictItem(10, "roasted_small_coffee",
            0x1A1612, DULL, round)

        ROASTED_LARGE_COFFEE = ORE_DICT_ITEMS.addOreDictItem(11, "roasted_large_coffee",
            0x1A1612, DULL, gemChipped)

        GROUND_COFFEE = ORE_DICT_ITEMS.addOreDictItem(12, "ground_coffee",
            0x1A1612, ORGANIC, dust)

        GRAHAM_CRACKER_DOUGH = ORE_DICT_ITEMS.addOreDictItem(13, "graham_cracker_dough",
            0xF0C25D, ORGANIC, crushed)

        GRAHAM_CRACKER_CHUNK = ORE_DICT_ITEMS.addOreDictItem(14, "graham_cracker_chunk",
            0xFFE1A1, DULL, plateDense)

        GRAHAM_CRACKER_SLICE = ORE_DICT_ITEMS.addOreDictItem(15, "graham_cracker_slice",
            0xFFE1A1, DULL, plate)

        ANIMAL_FAT = ORE_DICT_ITEMS.addOreDictItem(16, "animal_fat",
            0xFFF200, ORGANIC, ingot, "C57H110O6")

        ZEST_DUST = ORE_DICT_ITEMS.addOreDictItem(17, "zest",
            0xD8FF4A, FINE, dust)

        POPPY_DUST = ORE_DICT_ITEMS.addOreDictItem(18, "poppy",
            0x940801, ROUGH, dust)

        APPLE_PULP = ORE_DICT_ITEMS.addOreDictItem(19, "apple_pulp",
            0xCCCC99, NANOPARTICLES, dust)

        HARD_APPLE_CANDY_CHUNK = ORE_DICT_ITEMS.addOreDictItem(20, "hard_apple_candy_chunk",
            0x78E32B, ROUGH, plateDense)

        HARD_APPLE_CANDY_PLATE = ORE_DICT_ITEMS.addOreDictItem(21, "hard_apple_candy",
            0x78E32B, ROUGH, plate)

        HARD_APPLE_CANDY_DUST = ORE_DICT_ITEMS.addOreDictItem(22, "hard_apple_candy",
