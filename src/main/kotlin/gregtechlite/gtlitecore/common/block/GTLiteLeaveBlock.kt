...
class GTLiteLeaveBlock(private val offset: Int) : BlockLeaves(), TranslatableBlock
{
    ...
    fun registerColors()
    {
        Games.blockColors().registerBlockColorHandler({ blockState, worldIn, blockPos, tintIndex ->
            getTreeFromState(blockState).getBlockColor(blockState, worldIn, blockPos, tintIndex) }, this)

        Games.itemColors().registerItemColorHandler({ stack, tintIndex ->
            getTreeFromState(getStateFromMeta(stack.itemDamage))
                .getItemColor(stack, tintIndex) }, this)
    }
