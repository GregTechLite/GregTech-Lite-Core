...
class BlockBottlecrate : BlockHorizontal(Material.WOOD) {
    ...
    init {
        setRegistryName(MOD_ID,"bottlecrate")
        setTranslationKey("gtlitecore.bottlecrate")
        setHardness(2.0f)
        setResistance(5.0f)
    ...
    override fun getStateFromMeta(meta: Int): IBlockState {
        return defaultState.withProperty(FACING, EnumFacing.byHorizontalIndex(meta and 3))
    }
    ...
    override fun onBlockActivated(
        ...
        for (i in 0..8){
            ...
            val physIndex = getPhysIndex(i,state.getValue(FACING))
            val box = bottleBoxes[physIndex]
        ...
        if (!slotItem.isEmpty and !isFluidContainer(heldItem))
        {
        ...
        }else if (!heldItem.isEmpty and isFluidContainer(heldItem))
        {
    ...
    override fun addCollisionBoxToList(
        ...
        CRATE_AABB.forEachIndexed { i,box->
    ...
    override fun collisionRayTrace(
        ...
        CRATE_AABB.forEachIndexed lit@{ i,box->
            if ((i%2 == facing.horizontalIndex%2) or (i>=4)){
                val hit = rayTrace(pos, start, end, box)
                hit?.let {
                    val dist = hit.hitVec.squareDistanceTo(start)
    ...
    override fun breakBlock(world: World, pos: BlockPos, state: IBlockState) {
        ...
        if (te is Bottlecrate) {
            ...
            for (i in 0..8) {
                val stack: ItemStack = bottlecrate.getStackInSlot(i)
                if (!stack.isEmpty) {
