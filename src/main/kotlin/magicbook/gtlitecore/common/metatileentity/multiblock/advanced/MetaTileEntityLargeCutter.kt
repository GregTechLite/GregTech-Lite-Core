...
class MetaTileEntityLargeCutter(metaTileEntityId: ResourceLocation?) : MultiMapMultiblockController(metaTileEntityId, arrayOf(CUTTER_RECIPES, LATHE_RECIPES, POLISHER_RECIPES, SLICER_RECIPES))
{
    ...
    companion object
    {
        ...
        private val casingState
            get() = GTLiteMetaBlocks.METAL_CASING_01.getState(BlockMetalCasing01.MetalCasingType.MARAGING_STEEL_250)
    }
    ...
    private fun registerCasingMaps()
    {
        ...
        val maxLength = maxLength(listOf(motorCasing, conveyorCasing))
        motorCasings = consistent(motorCasing, maxLength)
        conveyorCasings = consistent(conveyorCasing, maxLength)
        hasRegistered = true
    }

    override fun createMetaTileEntity(tileEntity: IGregTechTileEntity) = MetaTileEntityLargeCutter(metaTileEntityId)
    ...
    override fun getMatchingShapes(): List<MultiblockShapeInfo>
    {
        ...
        StreamEx.of(motorCasings)
            .map { b ->
                builder.where('M', b)
                builder.where('O', conveyorCasings[count.get()])
    ...
    private inner class LargeCutterRecipeLogic(metaTileEntity: RecipeMapMultiblockController?) : MultiblockRecipeLogic(metaTileEntity)
    {
