...
class MassReplicationRecipeProducer
{

    companion object
    {

        // Common UUMatter in Gregtech is equivalence to "Bosonic" + "Fermionic" + electron.
        fun produce()
        {
            ...
            for (material in MaterialInfoCache.periodicTableMaterials)
            {
                ...
                // In reality world, for d-dim particle state space X, let Y be a new
                // state space which split it with all single quantum state with several
                // particles, then when d > 2, Y is simply connected. The basic group
                // of X is isomorphic of group action, and then:
                ...
                // This is an interesting pun: "half-integer" (spin number) and "mass % 2",
                // we ensure bosonic and fermionic by mass properties of elements... ^^
                val uuMatter = if (mass % 2 == 0) BosonicUUMatter.getFluid(mass) else FermionicUUMatter.getFluid(mass)
