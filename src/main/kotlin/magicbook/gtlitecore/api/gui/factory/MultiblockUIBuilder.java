...
import magicbook.gtlitecore.api.gui.drawable.GTObjectDrawable;
import magicbook.gtlitecore.api.gui.key.KeyManager;
import magicbook.gtlitecore.api.gui.key.Operation;
import magicbook.gtlitecore.api.gui.sync.UISyncer;
import magicbook.gtlitecore.api.utils.KeyUtil;
import magicbook.gtlitecore.api.utils.functions.ByteSupplier;
import magicbook.gtlitecore.api.utils.functions.FloatSupplier;
...
public class MultiblockUIBuilder
{
    ...
    public MultiblockUIBuilder structureFormed(boolean structureFormed)
    {
        ...
        if (!this.isStructureFormed)
        {
            ...
            IKey hover = KeyUtil.lang(TextFormatting.GRAY, "gregtech.multiblock.invalid_structure.tooltip");
            addKey(base, hover);
        }
        return this;
    }
    ...
    {
        if (idlingKey != null)
            this.idlingKey = IKey.lang(idlingKey).style(TextFormatting.GRAY);
        if (pausedKey != null)
            this.pausedKey = IKey.lang(pausedKey).style(TextFormatting.GOLD);
        if (runningKey != null)
            this.runningKey = IKey.lang(runningKey).style(TextFormatting.GREEN);
        return this;
    }
    ...
    public MultiblockUIBuilder addComputationUsageLine(int maxCWUt)
    {
        ...
        if (maxCWUt > 0)
        {
            IKey computation = KeyUtil.number(TextFormatting.AQUA, maxCWUt);
            addKey(KeyUtil.lang(TextFormatting.GRAY,
    ...
    public MultiblockUIBuilder addComputationUsageExactLine(int currentCWUt)
    {
        ...
        {
            IKey computation = KeyUtil.number(TextFormatting.AQUA, currentCWUt, " CWU/t");
            addKey(KeyUtil.lang(TextFormatting.GRAY,
    ...
    {
        if (!isStructureFormed || !ConfigHolder.machines.enableMaintenance)
            return this;
    ...
    public class InternalSyncer implements UISyncer
    {
        ...
        @Override
        @NotNull
        public <T> T syncObject(@NotNull Supplier<T> initial,
        ...
        @Override
        public <T, C extends Collection<T>> C syncCollection(C initial,
                                                             IByteBufSerializer<T> serializer,
                                                             IByteBufDeserializer<T> deserializer)
        {
            if (isServer())
        ...
        @Override
        public <T> T[] syncArray(T[] initial,
                                 IByteBufSerializer<T> serializer,
                                 IByteBufDeserializer<T> deserializer)
        {
            if (isServer())
