...
import magicbook.gtlitecore.api.gui.key.Operation;
import magicbook.gtlitecore.api.gui.theme.DisplayGuiTheme;
import magicbook.gtlitecore.api.gui.widget.ScrollableTextWidget;
import magicbook.gtlitecore.api.metatileentity.multiblock.ProgressBarMultiblock;
import magicbook.gtlitecore.api.utils.KeyUtil;
import magicbook.gtlitecore.api.utils.LambdaUtils;
...
public class MultiblockUIFactory
{
    ...
    private Widget<?> createIndicator(PanelSyncManager syncManager)
    {
        ...
        IDrawable indicator = new DynamicDrawable(() -> {
            if (!error.isEmpty())
            {
                return GTLiteMuiTextures.GREGTECH_LOGO_BLINKING_RED;
            }
            else if
            (!warning.isEmpty())
            {
                return GTLiteMuiTextures.GREGTECH_LOGO_BLINKING_YELLOW;
            }
            else
            {
    ...
    {
        this.warningText = merge ? LambdaUtils.mergeConsumers(this.warningText, warningText) : warningText;
        return this;
    }
     ...
     * This is called every tick on the client-side
     */
    public MultiblockUIFactory configureErrorText(boolean merge,
                                                  Consumer<MultiblockUIBuilder> errorText)
    {
        this.errorText = merge ? LambdaUtils.mergeConsumers(this.errorText, errorText) : errorText;
        return this;
    }
     ...
     * To use translation, use {@link KeyUtil#lang}
    ...
    {
        this.displayText = merge ? LambdaUtils.mergeConsumers(this.displayText, displayText) : displayText;
        return this;
    }
     ...
     * Called per tick on client side <br />
     * Each element of list is displayed on new line <br />
     * To use translation, use {@link KeyUtil#lang} or {@link KeyUtil#lang(String, Object...)}
     */
    ...
    {
        return buildUI(guiData, panelSyncManager, getDisplayBackground());
    }

    @NotNull
    public ModularPanel buildUI(PosGuiData guiData, PanelSyncManager panelSyncManager, UITexture displayTexture)
    {
        ModularPanel panel = GTGuis.createPanel(mte, width, height)
                .debugName("root_panel")
                .childIf(!disableDisplay, () -> createScreen(panelSyncManager, displayTexture));

        // TODO createExtras() hook for overrides?
        if (mte instanceof ProgressBarMultiblock
                && ((ProgressBarMultiblock) mte).getProgressBarCount() > 0)
        {
            ProgressBarMultiblock progressBarMultiblock = (ProgressBarMultiblock) mte;
        ...
        SlotGroupWidget playerInv = SlotGroupWidget.playerInventory(0);
        if (disableButtons)
        {
    ...
    {
        return createScreen(syncManager, getDisplayBackground());
    }

    protected Widget<?> createScreen(PanelSyncManager syncManager, UITexture displayTexture)
    {
    ...
    {
        return DisplayGuiTheme.getDisplayBackground(mte.getUITheme());
    }

    @NotNull
    protected Flow createButtons(@NotNull ModularPanel mainPanel,
                                 @NotNull PanelSyncManager panelSyncManager,
                                 PosGuiData guiData)
    {
        ...
        if (flexButton == null)
        {
            flexButton = new ToggleButton()
                    .debugName("flex_none")
                    .value(ALWAYS_ON)
                    .overlay(GTLiteMuiTextures.OVERLAY_NO_FLEX)
                    .size(18)
                    .addTooltipLine(IKey.lang("gregtech.multiblock.universal.no_flex_button"));
        }
        Widget<?> powerButton = createPowerButton(mainPanel, panelSyncManager);
    ...
    protected IWidget createDistinctButton(@NotNull ModularPanel mainPanel,
                                           @NotNull PanelSyncManager panelSyncManager)
    {
        if (!(mte instanceof IDistinctBusController) || !((IDistinctBusController) mte).canBeDistinct())
        {
            return new ToggleButton()
                    .debugName("distinct_none")
                    .value(ALWAYS_ON)
                    .size(18)
                    .overlay(GTLiteMuiTextures.OVERLAY_DISTINCT_BUSES[0])
                    .addTooltipLine(IKey.lang("gregtech.multiblock.universal.distinct_not_supported"));
        }
        IDistinctBusController distinct = (IDistinctBusController) mte;
        ...
        return new ToggleButton()
                ...
                .overlay(true, GTLiteMuiTextures.OVERLAY_DISTINCT_BUSES[1])
                .overlay(false, GTLiteMuiTextures.OVERLAY_DISTINCT_BUSES[0])
    ...
    /**
     * TODO Clean up reflect when mteClass open these methods.
     */
    protected IWidget createVoidingButton(@NotNull ModularPanel mainPanel, @NotNull PanelSyncManager panelSyncManager)
    {
        try
        {
            // shouldShowVoidingModeButton
            Class<?> mteClass = MultiblockWithDisplayBase.class;
            Method method = mteClass.getDeclaredMethod("shouldShowVoidingModeButton");
            method.setAccessible(true);
            boolean shouldShowVoidingModeButton = (boolean) method.invoke(mte);
            if (!shouldShowVoidingModeButton)
            {
                return new ToggleButton()
                        ...
                        .value(ALWAYS_ON)
                        .size(18)
                        .overlay(GTLiteMuiTextures.OVERLAY_VOID_NONE)
                        .addTooltipLine(IKey.lang("gregtech.gui.multiblock_voiding_not_supported"));
            }
            // getVoidingMode/setVoidingMode
            Method getVoidingMode = mteClass.getDeclaredMethod("getVoidingMode");
            getVoidingMode.setAccessible(true);
            Method setVoidingMode = mteClass.getDeclaredMethod("setVoidingMode", int.class);
            setVoidingMode.setAccessible(true);
            IntSyncValue voidingValue = new IntSyncValue(
                    () -> {
                        try
                        {
                            return (int) getVoidingMode.invoke(mte);
                        }
                        catch (Exception exception)
                        {
                            throw new RuntimeException("Failed to access and call 'getVoidingMode' method in MultiblockWithDisplayBase class", exception);
                        }
                    },
                    mode -> {
                        try
                        {
                            setVoidingMode.invoke(mte, mode);
                        }
                        catch (Exception exception)
                        {
                            throw new RuntimeException("Failed to access and call 'setVoidingMode' method in MultiblockWithDisplayBase class", exception);
                        }
                    }
            );

            // getVoidingModeTooltip
            Method getVoidingModeTooltip = mteClass.getDeclaredMethod("getVoidingModeTooltip", int.class);
            getVoidingModeTooltip.setAccessible(true);
            return new CycleButtonWidget()
                    .debugName("voiding_button")
                    .size(18)
                    .value(voidingValue)
                    .length(4)
                    .stateOverlay(0, GTLiteMuiTextures.MULTIBLOCK_VOID[0])
                    .stateOverlay(1, GTLiteMuiTextures.MULTIBLOCK_VOID[1])
                    .stateOverlay(2, GTLiteMuiTextures.MULTIBLOCK_VOID[2])
                    .stateOverlay(3, GTLiteMuiTextures.MULTIBLOCK_VOID[3])
                    .tooltipBuilder(t -> {
                        try
                        {
                            t.addLine(IKey.lang((String) getVoidingModeTooltip.invoke(null, voidingValue.getIntValue())));
                        }
                        catch (Exception exception)
                        {
                            t.addLine(IKey.lang("gregtech.gui.voiding_mode.error")); // TODO Add a error lang text.
                        }
                    });
        }
        catch (Exception exception)
        {
            return new ToggleButton()
                    .debugName("voiding_error")
                    .value(ALWAYS_ON)
                    .size(18)
                    .overlay(GTLiteMuiTextures.OVERLAY_VOID_NONE)
                    .addTooltipLine(IKey.lang("gregtech.gui.voiding_mode.error")); // TODO Add a error lang text.
        }
    ...
    protected Widget<?> createPowerButton(@NotNull ModularPanel mainPanel, @NotNull PanelSyncManager panelSyncManager)
    {
        ...
        Icon detail = GTLiteMuiTextures.BUTTON_POWER_DETAIL.asIcon().size(18, 6).marginTop(24);

        return new ToggleButton()
                .debugName("power_button")
                .size(18)
                .disableHoverBackground()
                .overlay(true, detail, GTLiteMuiTextures.BUTTON_POWER[1])
                .overlay(false, detail, GTLiteMuiTextures.BUTTON_POWER[0])
