...
public abstract class MetaTileEntityDelegator extends MetaTileEntity implements IDelegator
{
    ...
    protected final Predicate<Capability<?>> capabilityFilter;
    protected final int baseColor;

    public MetaTileEntityDelegator(ResourceLocation metaTileEntityId,
                                   Predicate<Capability<?>> capabilityFilter, int baseColor)
    {
    ...
    protected <T> T getDelegatedCapability(Capability<T> capability, EnumFacing side)
    {
        ...
        if (te == null || (te instanceof MetaTileEntityHolder
                && ((MetaTileEntityHolder) te).getMetaTileEntity() instanceof IDelegator))
            return null;
                                     ...
                                     IVertexOperation[] pipeline)
    {
        IVertexOperation[] colouredPipeline = ArrayUtils.add(pipeline,
                new ColourMultiplier(GTUtility.convertRGBtoOpaqueRGBA_CL(
                        this.getPaintingColorForRendering())));
        for (EnumFacing facing : EnumFacing.values())
            Textures.renderFace(renderState, translation, colouredPipeline, facing, Cuboid6.full,
                    this.getBaseTexture(), BlockRenderLayer.CUTOUT_MIPPED);
    }
